...
import static com.uber.athenax.tests.ITestUtil.*;
...
public class StartJobITest {
    ...
    public void testRemoveJob(){
        Configuration.getDefaultApiClient().setBasePath(String.format("http://localhost:%d%s", 18080, WebServer.BASE_PATH));
        JobsApi api = new JobsApi();
        UUID uuid = UUID.fromString("7c8d915b-8db6-45ec-9f93-a998db39dfe5");
        try {
//            List<ExtendedJobDefinition> list = api.listJob();
//            for(ExtendedJobDefinition job: list){
//                System.err.println(job.getUuid());
                api.removeJob(uuid);
//            }
    ...
    @Test
    public void testDeployJob() throws Exception {
        Configuration.getDefaultApiClient().setBasePath(String.format("http://localhost:%d%s", 18080, WebServer.BASE_PATH));
        JobsApi api = new JobsApi();
        ...
        JobDefinition job = new JobDefinition()
                .query("SELECT * FROM input.foo1")
                .addDesiredStateItem(state);
        api.deployJob(UUID.fromString(uuid), job);
    }
